...
void calcPrompt() {
  cout << "[{\"name\":\"Calculator - Type "
          ...
          ";)\\\"\"}]";
  exec("eww update winsearch_actions='{\"name\":\"Calculator - Type "
       "something!\",\"icon\":\"images/svg/dark/"
       "calculator.svg\",\"exec\":\"wl-copy \\\"Clipboard contents "
       ";)\\\"\"}' &");
...
void tryThemeCmd() {
  ...
  for (int i = 0; i < entryNames.size(); i++) {
    ...
    cout << '{';
    cout << "\"name\":\"" << entryName << "\",\"exec\":\">load " << entryName
         << "\"";
    cout << '}';
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 2 && string(argv[2]) == "--updateinfo")
    updateInfo = true;
  searchTerm = argv[1];

  // Special commands
  if (searchTerm == "--calculator")
    calcPrompt();
  if (searchTerm[0] == '>') {
